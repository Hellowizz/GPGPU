...
namespace IMAC
{
	...
	void sumMatrixCPU(const int l, const int h, const int *const a, const int *const b, int *const c)
	{
		std::cout << "Addition on CPU (sequential) of two matrix"	<< std::endl;
		ChronoCPU chrCPU;
		chrCPU.start();
		for (int x=0; x < l; ++x){
			for(int y=0; y < h; ++y){
	...
	{	
		int L = DEFAULT_VECTOR_SIZE;
		int H = DEFAULT_VECTOR_SIZE;
		...
		for (int i = 1; i < argc; ++i ) 
		{
			if (!strcmp( argv[i], "-n")) // Arrays size
			{
				...
				if (sscanf(argv[i+2], "%d", &H) != 1)
				{
					printUsageAndExit(argv[1]);
				}
			}
			else
			{
		...
		std::cout << "Summing vectors of length " << L << " and high " << H << std::endl << std::endl;

		ChronoCPU chrCPU;

		// Allocate arrays on CPU
		std::cout 	<< "Allocating input (3 arrays): " 
					<< ( ( L * H * 3 * sizeof( int ) ) >> 20 ) << " MB on Host" << std::endl;
		chrCPU.start();
		int *a = new int[L*H];
		int *b = new int[L*H];
		int *resCPU = new int[L*H];
		chrCPU.stop();
		std::cout 	<< " -> Done : " << chrCPU.elapsedTime() << " ms" << std::endl << std::endl;
		...
		int *resGPU = new int[L*H];
		// Call student's code
		studentJob(L, H, a, b, resGPU);
		
		std::cout << "============================================"	<< std::endl << std::endl;

		std::cout << "Checking result..." << std::endl;
		if (compare(resCPU, resGPU, L*H))
		{
