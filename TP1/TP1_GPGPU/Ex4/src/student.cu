...
namespace IMAC
{
	__global__ void sumMatrixCUDA(const int h, const int l, const int *const dev_a, const int *const dev_b, int *const dev_res)
	{
		// id global
		...
		for (int idX = idThreadG; idX < h * l; idX += nbThreadsG)
		{
			dev_res[idX] = dev_a[idX] + dev_b[idX];
		}
	}

    void studentJob(const int h, const int l, const int *const a, const int *const b, int *const res)
	{
		ChronoGPU chrGPU;
		...
		const size_t bytes = h * l * sizeof(int);
		...
		// Launch kernel
		int nbThreads = 100;
		sumMatrixCUDA<<<2, nbThreads>>>(h, l, dev_a, dev_b, dev_res);
		chrGPU.stop();
